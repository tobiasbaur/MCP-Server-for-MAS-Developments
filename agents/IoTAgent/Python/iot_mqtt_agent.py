...
class LocalFileHandler:
    ...
    def __init__(self, base_name, local_dir, file_type, size_limit, remote_subdir, config, language_code):
        ...
        """
        self.base_name = base_name# iot_mqtt_agent.py
...
def generate_logical_sentence(parameters, language_code, config, max_retries=3, wait_seconds=5):
    ...
    while attempt < max_retries:
        try:
            # Erstellen des reinen Prompts (Inhalt, der später im content-Feld liegt)
            prompt = (
                "Erstelle einen logischen Satz aus den folgenden json Parametern:\n"
                + json.dumps(parameters, ensure_ascii=False, indent=4)
            )
            ...
            # ------------------------------------------------------------------
            # NEU: FIPA-ACL Nachricht konstruieren
            # ------------------------------------------------------------------
            # Beispielhaftes ACL-Format im JSON-Gewand
            acl_payload = {
                "performative": "request",
                "sender": "IoT_MQTT_Agent",
                "receiver": "Chatbot_Agent",
                "language": "fipa-sl",   # oder "de"/"en" oder was dein Chatbot versteht
                "ontology": "fujitsu-iot-ontology",
                "content": {
                    # Hier legen wir die alten Felder ab, also quasi den eigentlichen Prompt
                    "question": prompt,
                    "usePublic": True,    # wie bisher
                    "groups": [],
                    "language": language_code
                }
            }
            ...
            # Wichtig: Wir schicken jetzt acl_payload statt des alten "payload"
            response = requests.post(
                config['chatbot_agent']['api_url'],
                json=acl_payload,
                headers=headers,
            ...
            chatbot_response_message = languages[language_code]["chatbot_response"].format(
            ...
            if response.status_code == 200:
                data = response.json()
                # Wir gehen davon aus, dass auch jetzt noch 'content' zurückkommt
                generated_sentence = data.get("content", "").strip('"')
                if generated_sentence:
                ...
                if not generated_sentence:
                    ...
                    logging.warning(warning_message)
                    raise ValueError("Empty 'content' field in the response.")
                return generated_sentence
            else:
                error_message = languages[language_code]["chatbot_error_status"].format(
            ...
            attempt += 1
            communication_error_message = languages[language_code]["communication_error"].format(
                e=e, attempt=attempt, max_retries=max_retries
            )
            logging.error(communication_error_message)
            if attempt < max_retries:
                waiting_message = languages[language_code]["waiting_before_retry"].format(
